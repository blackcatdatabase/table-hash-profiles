...
namespace BlackCat\Database\Packages\HashProfiles\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto;
use BlackCat\Database\Packages\HashProfiles\Mapper\HashProfileDtoMapper;
 ...
 */
final class HashProfilesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private HashProfileRepository $hashProfileRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'hash_profiles' => $this->hashProfileRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\HashProfiles\ModuleException("Record {$idStr} not found for locking.");
            }
