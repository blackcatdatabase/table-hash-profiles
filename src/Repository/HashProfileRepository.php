...
namespace BlackCat\Database\Packages\HashProfiles\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\HashProfiles\Definitions;
use BlackCat\Database\Packages\HashProfiles\Criteria;
use BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto as Dto;
use BlackCat\Database\Packages\HashProfiles\Mapper\HashProfileDtoMapper as RowMapper;
...
final class HashProfileRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\HashProfiles\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'algo_id', 'output_len', 'params', 'status' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\HashProfiles\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'algo_id', 'output_len', 'params', 'status' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\HashProfiles\Mapper\HashProfileDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\HashProfiles\Dto\HashProfileDto|null {
