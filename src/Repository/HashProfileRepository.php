...
use BlackCat\Database\Packages\HashProfiles\Repository\HashProfileRepositoryInterface;
...
class HashProfileRepository implements HashProfileRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\HashProfiles\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'algo_id', 'output_len', 'params', 'status' ]);
          return count($rows);
        }
